(function () {
    ...
    function validarFiltros(){
        var filtros = {
            empresaPrestadora : $('#cd_empresa_prestadora_combo').val(),
            status : $('#st_situacao_combo').val(),
            tipo : $('#cd_tipo_empregado_combo').val(),
            matricula : $('#input_matricula').val(),
            cpf : $('#input_cpf').val(),
            nome : $('#input_nome').val(),
            empresa : $('#cd_empresa_combo').val()
        };
        
        var temFiltroPreenchido = Object.values(filtros).some(function(valor){
            return valor.trim() !== "";
        });
    ...
    function pesquisar_usuarios(){
        if(!validarFiltros()){
            helper.alertError('Use pelo menos 1 filtro!');
            return false;
        }
      ...
      function populateTableUsuarios(retorno){
        ...
        var rota_pesquisar_chamados_rapido_usuario   = $("#input-rota-pesquisar-chamado-rapido-usuario").val();
        var rota_pesquisar_chamados_usuario   = $("#input-rota-pesquisar-chamado-usuario").val();
        ...
        $('#tb-usuarios').DataTable({
            ...
            "columns": [
                ...
                {
                    "render": function (data, type, row, meta) {
                        if(row.no_social === null || row.no_social === ""){
                            return row.fdenome
                        }else{
                            return row.no_social
                        }
                    }
                },
                {
                ...
                },
                {
                    "render": function (data, type, row, meta) {
                        ...
                        var sqPesquisarChamadosRapido     = rota_pesquisar_chamados_rapido_usuario + '/' + row.fdematric;
                        var sqPesquisarChamados     = rota_pesquisar_chamados_usuario + '/' + row.fdematric;
                        ...
                        var pesquisarChamadosRapido  = '<a class="btn btn-primary btn-sm" data-href="' + sqPesquisarChamadosRapido + '" id="btnPesquisaRapida"><i class="fa fa-bars" data-toggle="tooltip" data-placement="top" title="Pesquisar Chamados Rápido"></i></a>&nbsp;';
                        var pesquisarChamados  = '<a class="btn btn-primary btn-sm" data-href="'+ sqPesquisarChamados +'" value="'+ row.fdematric+row.fdedgv +'"><i class="fa fa-edit" data-toggle="tooltip" data-placement="top" title="Pesquisar Chamados" id="pesquisarChamados"></i></a>&nbsp;';
                        ...
                        if(row.fdesituacao == 1){
                            var botoes = visualizar + editar + pesquisarChamados + pesquisarChamadosRapido  + status;
                        }else{

                            var botoes = visualizar + editar + pesquisarChamadosRapido  + status;
                        }
